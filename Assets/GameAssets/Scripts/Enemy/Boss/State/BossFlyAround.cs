...
public class BossFlyAround : BossState
{
    ...
    private float timer;
    private float timerToArrive = 300f; // 수정
    ...
    private void patternSelectByPercentage(int numberToSelect)
    {
        ...
        else if(numberToSelect == 1) // 일직선 상 먼 두 지점
        {
            ...
            else if (60 < randomNumber && randomNumber <= 80) // 25% 확률로 활강 돌진
            {
                patternSelectAndChangeState(1);
            }
            else// 폭탄 투하
